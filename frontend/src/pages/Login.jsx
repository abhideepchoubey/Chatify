  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      <label className="block">
        <span className={["mb-2 block text-sm font-medium", labelTone].join(" ")}>
          Username
        </span>
        <input
          type="text"
          ...
          className={["w-full rounded-[22px] border px-4 py-3.5 outline-none transition", inputTone].join(" ")}
        />
      </label>
...
export default function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (authModalMode === "register" && form.password !== form.confirmPassword) {
  ...
  return (
    <>
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl items-center gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
          <section className="relative pt-16 lg:pt-10">
            ...
            <div className="mt-10 max-w-3xl">
              <p className={["text-sm uppercase tracking-[0.38em]", tones.subtle].join(" ")}>
                Private chatting like WhatsApp
              </p>
              <h1 className={["mt-4 font-display text-5xl font-semibold leading-[1.02] sm:text-6xl xl:text-7xl", isLight ? "text-slate-950" : "text-white"].join(" ")}>
                Chatify keeps conversations private, expressive, and instantly realtime.
              </h1>
              <p className={["mt-6 max-w-2xl text-base leading-8 sm:text-lg", tones.muted].join(" ")}>
                Add friends, create groups, upload photos, and move through direct chats with the same comfort and speed people love in modern private messaging apps.
              </p>
            </div>
            ...
            <div className="mt-10 grid gap-4 md:grid-cols-3">
              {features.map((feature, index) => (
                ...
                >
                  <p className={["text-xs uppercase tracking-[0.28em]", tones.subtle].join(" ")}>
                    What Chatify Offers
                  </p>
                  <h2 className={["mt-3 font-display text-xl font-semibold", isLight ? "text-slate-950" : "text-white"].join(" ")}>
                    {feature.title}
                  </h2>
                  <p className={["mt-3 text-sm leading-6", tones.muted].join(" ")}>
                    {feature.body}
                  </p>
                </div>
              ))}
          ...
          <section className="relative">
            <div className={["relative overflow-hidden rounded-[34px] border p-5 backdrop-blur-xl sm:p-6", tones.heroCard].join(" ")}>
              <div
                className={[
              ...
              <div className="relative flex items-center justify-between">
                <div>
                  <p className={["text-xs uppercase tracking-[0.3em]", tones.subtle].join(" ")}>
                    Private Messaging Preview
                  </p>
                  <h2 className={["mt-2 font-display text-2xl font-semibold", isLight ? "text-slate-950" : "text-white"].join(" ")}>
              ...
              <div className={["mt-5 space-y-3 rounded-[28px] border p-4", isLight ? "border-slate-200 bg-slate-50/90" : "border-white/10 bg-slate-950/65"].join(" ")}>
                {mockMessages.map((message, index) => (
                  ...
                  >
                    ...
                    >
                      <p className={["mb-1 text-[11px] font-semibold uppercase tracking-[0.2em]", message.side === "right" ? "text-white/70" : tones.subtle].join(" ")}>
                        {message.sender}
                      </p>
                      <p>{message.text}</p>
                    </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                <div className={["rounded-[24px] border p-4", tones.heroCard].join(" ")}>
                  <p className={["text-xs uppercase tracking-[0.3em]", tones.subtle].join(" ")}>
                    Secure feel
                  </p>
                  <p className={["mt-2 text-sm leading-6", tones.muted].join(" ")}>
                    Fast direct rooms, friend-based access, and persistent conversations with clean UI hierarchy.
                  </p>
                </div>
                <div className={["rounded-[24px] border p-4", tones.heroCard].join(" ")}>
                  <p className={["text-xs uppercase tracking-[0.3em]", tones.subtle].join(" ")}>
                  ...
                  <p className={["mt-2 text-sm leading-6", tones.muted].join(" ")}>
                    Share photos, create groups, and keep every message flowing in realtime without leaving the thread.
                  </p>
                </div>
              </div>
            </div>
      ...
      <ModalShell
        ...
        onClose={closeModal}
        title={authModalMode === "register" ? "Create Your Account" : "Welcome Back"}
        description={
      ...
      >
        <div className={["rounded-[24px] border p-4 sm:p-5", tones.panel].join(" ")}>
          <AuthForm
